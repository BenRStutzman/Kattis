...
def main():
    ...
    matrix2 = []
    for node, neighbors in enumerate(matrix):
        secondary = set()
        for neighbor in neighbors:
            secondary = secondary.union(matrix[neighbor])
        matrix2.append(secondary)
    '''
    for neighbors in matrix:
        for neighbor in neighbors:
            i += 1
            matrix2[neighbor] = matrix2[neighbor].union(neighbors)
    '''
